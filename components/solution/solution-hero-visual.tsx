  ...
  return (
    <div className="relative isolate">
      ...
      <div className="relative rounded-[2rem] border border-white/10 bg-[oklch(0.14_0.03_230)] p-4 shadow-2xl shadow-emerald-500/10 sm:p-5">
        ...
        {/* volume widget */}
        <div className="mt-3.5 flex items-end justify-between border-b border-white/5 pb-3.5">
          <div>
            <p className="text-[11px] font-medium uppercase tracking-[0.12em] text-white/50">{volumeLabel}</p>
            <p className="mt-0.5 font-[family-name:var(--font-serif,var(--font-sans))] text-2xl font-semibold tracking-tight text-white sm:text-3xl">
              {volumeValue}
            </p>
          </div>
          <div className="flex items-end gap-1 pb-1">
            {[16, 22, 14, 28, 20, 34, 24, 40, 32].map((h, i) => (
              <span
                ...
                style={{ height: `${Math.round(h * 0.65)}px`, opacity: 0.4 + i * 0.07 }}
              />
            ))}
          </div>
        </div>
        ...
        <ul className="mt-3 space-y-1.5">
          {messages.map((m, i) => {
            ...
            return (
              <li
                key={m.sender + i}
                className="group flex gap-2 rounded-xl border border-white/5 bg-white/[0.03] p-2 backdrop-blur-sm transition hover:border-primary/30 hover:bg-white/[0.06]"
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 truncate text-[13px] leading-snug text-white/75">{m.body}</p>
                  <div className="mt-0.5 flex items-center gap-2 text-[10px] font-medium uppercase tracking-wider">
