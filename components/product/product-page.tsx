...
export function Section({
  ...
  return (
    <section
      id={id}
      className={`${toneClass} py-14 sm:py-16 ${screenHeight ? "flex min-h-screen flex-col justify-center" : ""} ${className}`}
    >
...
export function Faq({
  ...
  const grid = (
    <div className={`${title ? "mt-10" : ""} columns-1 gap-4 md:columns-2`}>
      {items.map((item) => (
        <details
          key={item.q}
          className="group mb-4 break-inside-avoid-column rounded-xl border border-border bg-card p-5 shadow-sm open:border-primary/30"
        >
