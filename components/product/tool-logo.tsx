...
export function ToolLogo({ name, url, logoUrl }: { name: string; url: string; logoUrl?: string }) {
  const hostname = (() => {
  ...
  const sources = [
    ...
    ...(hostname ? [
      ...
      // Google's high-res icon API — always returns something at 256px
  ...
  if (idx >= sources.length || !hostname) {
    return (
      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-[16px] font-black ${colorFor(name)}`}>
