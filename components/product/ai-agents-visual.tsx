          ...
          {TURNS.map((turn, i) => {
            if (turn.role === "customer") {
              return (
                ...
                >
                  <span className="mt-1 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white/10 text-white/70">
                    <UserRound className="h-3.5 w-3.5" />
                  </span>
                  <div className="min-w-0 flex-1 rounded-2xl rounded-tl-md border border-white/5 bg-white/[0.04] px-3.5 py-2.5">
                    ...
                    <p className="mt-1 text-[12.5px] leading-snug text-white/85">{turn.text}</p>
                  </div>
                </li>
              )
            }
            if (turn.role === "ai") {
              return (
                <li
                  ...
                  style={{ animation: `agentTurnIn 0.5s ease-out ${i * 0.08}s backwards` }}
                >
                  <div className="min-w-0 max-w-[88%] rounded-2xl rounded-tr-md border border-primary/30 bg-gradient-to-br from-primary/25 to-primary/10 px-3.5 py-2.5 backdrop-blur-sm">
                    <div className="flex items-center justify-between gap-3 text-[10.5px]">
                    ...
                    <p className="mt-1 text-[12.5px] leading-snug text-white">{turn.text}</p>
                    {turn.source ? (
            ...
            return (
              <li
                ...
                style={{ animation: `agentTurnIn 0.5s ease-out ${i * 0.08}s backwards` }}
              >
                <div className="min-w-0 max-w-[88%] rounded-2xl rounded-tr-md border border-amber-400/30 bg-amber-400/10 px-3.5 py-2.5">
                  <div className="flex items-center justify-between gap-3 text-[10.5px]">
                ...
                <span className="mt-1 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-amber-400/20 text-amber-300">
                  <UserRound className="h-3.5 w-3.5" />
                </span>
              </li>
            )
