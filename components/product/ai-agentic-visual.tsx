...
import { Bot, Check, Cpu, Globe, Loader2, MessageCircle, MessageSquareText, Sparkles } from "lucide-react"

type Step = { label: string; status: "done" | "running" | "pending" }
type Run = { goal: string; steps: Step[] }

const RUNS: (Omit<Run, "steps"> & { steps: Omit<Step, "status">[] })[] = [
  {
    goal: "Qualify and follow up with new leads",
    steps: [
      { label: "Fetched 24 new leads from HubSpot" },
      { label: "Scored by fit: 8 high-priority" },
      { label: "Drafted personalized WhatsApp messages" },
      { label: "Sent to 8 leads via SMSLocal" },
      { label: "Logged results back to CRM" },
    ],
  },
  {
    goal: "Handle refund tickets automatically",
    steps: [
      { label: "Pulled 12 open refund tickets" },
      { label: "Verified order status via API" },
      { label: "Approved 9 eligible refunds" },
      { label: "Sent confirmation SMS to customers" },
      { label: "Escalated 3 edge cases to agent" },
    ],
  },
  {
    goal: "Send appointment reminders for today",
    steps: [
      { label: "Fetched today's calendar from Sheets" },
      { label: "Found 31 upcoming appointments" },
      { label: "Personalised reminder text for each" },
      { label: "Dispatched via SMS & WhatsApp" },
      { label: "Tracked delivery receipts" },
    ],
  },
]
